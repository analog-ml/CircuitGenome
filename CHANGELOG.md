...
- ♻️ Derive the circuit's structure once, in one place — `shared/circuit_view.py` owns `CircuitView` and `analyze_circuit(fbr, topology)`; the four extraction steps behind it (`extract_slot_transistors`, `extract_slot_resistors`, `deduplicate_devices`, `check_topology_match`) become private to it, so a sizer asks once instead of making four calls in the right order. The gm/Id pipeline's `GmIdCircuitView` extends it with the block decomposition and cascode refs, keeping `gmid/blocks.py` out of the shared core. Orphan adoption now runs for both sizers rather than only gm/Id ([#XXX](https://github.com/analog-ml/CircuitGenome/pull/XXX)).
