...
- 🐛 A single stage's output node is resolved structurally, not assumed to be the input pair's drain — on a folded or telescopic cascode load the pair's drain is the cascode *source*, a ~`1/gm` node, so `rout` and the load conductance were both read one device too low. `psrr_db` was withheld on 5 of the catalog's 10 `load` variants (no device on the assumed node for the four cascode families; no device at all for the two resistor ones, where the answer is exactly `1/R` and the sizer had already solved for it), and `gain_db` under-reported cascode single stages by 12–51 dB — enough to rank a telescopic cascode below a plain mirror load. The output node is now found as the end of the input pair's signal path, and the load branch's conductance is read cascode-aware, so stacking a load improves its PSRR instead of appearing to degrade it. All 10 variants report PSRR; 64 of 80 single-stage `psrr_db` values and 48 of 80 `gain_db` values change, every multi-stage number is byte-identical. Phase margin still withholds on the two load families whose non-dominant pole this model cannot place, now documented as a decision ([#228](https://github.com/analog-ml/CircuitGenome/issues/228)).
