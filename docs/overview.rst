...
.. note::
   ...
   A ``*_buffered_*`` template is identical to its non-buffered counterpart but
   adds a **source-follower output stage** after the final gain stage.  The
   follower acts as a unity-gain buffer that drives heavier loads without
   loading the gain path; it does not change the number of gain stages or the
   compensation scheme.  See the :doc:`Topology Synthesizer module page
   <modules/synthesizer>` for the full output-stage catalogue.

.. rubric:: Circuits generated per template

A **topology template** fixes the *skeleton* of an op-amp — which module slots
it has and how they are wired — but not which variant fills each slot.
Enumerating a template fills every slot with each compatible module variant and
counts the combinations that form a valid circuit, so a single template expands
into the many concrete op-amps counted below.  For how each count arises — the
compatibility filtering that prunes non-functional combinations and the
per-template derivation — see the :doc:`Topology Synthesizer module page
<modules/synthesizer>`.
