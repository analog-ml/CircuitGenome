...
variants excluded — see the tags discussed further down).  It is generated by
``tools/gen_topology_counts.py`` and refreshed whenever the enumeration
changes; the prose that follows explains how each count arises. (The NMC
templates enumerated zero until the non-inverting current-mirror gm2 stage was
added in issue #139; see the compensation parity discussion below.)
