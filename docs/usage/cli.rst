...
optionally, maps them to a topology's slots.

.. code-block:: bash

   # SR only — identify structures without topology context
   circuitgenome recognize path/to/circuit.ckt

   # SR + FBR — also assign structures to topology slots
   circuitgenome recognize path/to/circuit.ckt \
     --topology two_stage_opamp_single_ended

Sample output (SR + FBR)::
...
.. list-table::
   ...
   * - ``NETLIST``
     ...
     - (required)
   * - ``--topology NAME``
     - Topology name for FBR slot assignment; omit for SR-only output
     - (SR only)
