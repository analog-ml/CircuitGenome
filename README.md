...
Takes a flat SPICE netlist and identifies structural subcircuits (differential pairs, current mirrors, cascode loads, bias generators, compensation networks, second-stage amplifiers, etc.) via a YAML pattern library. The library covers all 30 reachable variants across `one_stage_opamp` and `two_stage_opamp_single_ended` — 24 patterns across four base categories (input pair, load, tail current, bias generation) plus 6 new patterns for compensation and second-stage; broader topology coverage is planned.
